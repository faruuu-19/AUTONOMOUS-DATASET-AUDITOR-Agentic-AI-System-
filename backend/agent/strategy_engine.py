...
class AutonomousStrategyEngine:
    ...
    def decide_audit_strategy(self, profile: DatasetProfile) -> Tuple[List[str], Dict[str, str]]: 
        ...
        _safe_print(f"   Dataset Profile: {profile.num_rows} rows, {profile.num_features} features") 
        _safe_print(f"   Complexity Score: {profile.complexity_score:.2f}") 
        _safe_print(f"   Class Balance: {profile.class_balance_ratio:.2f}") 
        
        # NEW: Get learned recommendations from meta-learner
        learned_recs = self.meta_learner.get_learned_recommendations(profile.to_dict())
        ...
        for tool, boost in learned_recs.get('tool_boosts', {}).items():
            if tool in tool_scores:
                ...
                _safe_print(f"   ⬆️  Boosting {tool}: {original_score:.2f} → {tool_scores[tool]:.2f} (learned pattern match)")
        ...
        if learned_recs.get('optimal_sequence'):
            _safe_print(f"   🧠 Using learned optimal sequence")
            ordered_tools = [tool for tool in learned_recs['optimal_sequence'] if tool in tool_scores]
        ...
        if learned_recs.get('skip_threshold'):
            ...
            _safe_print(f"   🎚️  Using learned skip threshold: {SKIP_THRESHOLD:.2f}")
        ...
        _safe_print(f"   ⊘ Skipping: {skipped_tools if skipped_tools else 'None'}") 
        _safe_print(f"\n   🎯 Execution Order (by priority):") 
        for i, tool in enumerate(tools_to_run, 1): 
            _safe_print(f"      {i}. {tool} (score: {tool_scores[tool]:.2f})") 
    ...
    def record_audit_outcome(self, profile: DatasetProfile, 
        ...
        _safe_print(f"   Tools executed: {len(tools_executed)}")
        _safe_print(f"   Critical findings: {total_critical}")
        _safe_print(f"   Total history: {len(self.audit_history)} audits")
    ...
    def load_history(self):
        ...
        try:
            ...
            _safe_print(f"📚 Loaded {len(self.audit_history)} historical audits for autonomous learning")
        except Exception as e:
            _safe_print(f"⚠️  Could not load history: {e}")
